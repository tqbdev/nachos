...
class OpenFile {
  public:
    OpenFile(int f) { file = f; currentOffset = 0; type = 1;}	// open the file default type = 1, read-only file
    ...
    int Seek(int pos) { Lseek(file, pos, 0); return Tell(file); }
    
    //void SetIndex(int index) { idx = index; }
    //int GetIndex() { return idx; }
    
    int GetType() { return type; }
    int GetCurrentPos() { return currentOffset; }
  private:
    int file;
    	// type 1: only read
...
class OpenFile {
  public:
    ...
    OpenFile(int sector, int type);	// Same default constructor but have type
    					// type 1: only read
					// type 0 : read and write
